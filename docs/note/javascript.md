...
- 全局变量 如果未定义 请用window.variable调用 如果不加window可导致报错，阻塞后续代码执行
...
- 生成随机字符串 `Math.random().toString(32) // 0.3s22mrmvjog` 去掉前两位 `.slice(2)`
...
- 字符串转数字
  ...
  4. parseFloat(一个参数);  
  5. NumberObject.toFixed(n);n为0到20之间值(用Number(参数)来创建数值对象，不推荐用new Number());（例：Number(15.265124).toFixed(2) // 15.27 四舍五入）；
...
- document.documentElement.clientHeight | clientWidth 包括内边距（兼容性好）
...
#### 静态属性和方法
- 静态属性： 在属性前加 static(草案阶段) 或是  ClassName.propertyName = 'xx' 方式设置
- 在方法前加 static
...
const { a: a, b:b=9 } = { a: 12, b: null } 
console.log(a, b); // 12, null
// 当被赋予的值是null时，是不会用默认值将其覆盖的，默认值只会赋值为undefined的情况
